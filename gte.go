...
func (n *Inner) getGTE(
	...
	//pp("%p top of getGTE(), path='%v'; we are '%v' %v;  smallestWillDo=%v", n, string(n.path), n.FlatString(depth, 0), n.rangestr(), smallestWillDo)

	//defer func() {
	//pp("%p returning from calldepth=%v getGTE value='%v', found='%v'; dir='%v'; id='%v'; my Inner %v;  smallestWillDo=%v; pathMatch=%v", n, calldepth, value, found, dir, id, n.rangestr(), smallestWillDo, pathMatch)
	//}()
